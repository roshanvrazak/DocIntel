  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      while (true) {
        ...
        // Split on sentinel — text before it is the response, after is citation JSON
        if (accumulated.includes(CITATION_SENTINEL)) {
          const [textPart, citationRaw] = accumulated.split(CITATION_SENTINEL);
          let citations: CitationData[] | undefined;
          try {
            citations = JSON.parse(citationRaw) as CitationData[];
          } catch {
            citations = undefined;
          }
          setMessages((prev) =>
            prev.map((m) =>
              m.id === assistantMessageId ? { ...m, content: textPart, citations } : m
            )
          );
        } else {
          setMessages((prev) =>
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-100 flex flex-wrap gap-3">
                    {m.citations.map((c, i) => (
                      ...
                      >
                        ...
                        {c.filename ? `${c.filename} p.${c.page_number}` : `Source [${i + 1}]`}
                      </button>
                    ))}
                  </div>
                )}
